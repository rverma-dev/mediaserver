...
# Telegram bot token: write to token file if set in .env
if [[ -n "${TELEGRAM_BOT_TOKEN:-}" ]]; then
    TOKEN_DIR="${MEDIASERVER_ROOT}/config/openclaw"
    mkdir -p "$TOKEN_DIR"
    echo -n "$TELEGRAM_BOT_TOKEN" > "${TOKEN_DIR}/telegram-bot-token"
    chmod 600 "${TOKEN_DIR}/telegram-bot-token"
    info "Telegram bot token written to ${TOKEN_DIR}/telegram-bot-token"
fi

info "Nix setup complete. Services: systemctl --user status caddy openclaw-gateway"
